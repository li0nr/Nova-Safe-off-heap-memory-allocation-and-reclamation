...
//--------------------------------------------------------------------------------
   ...
   final Node<K,V> root;
   final CopyConstructor<K> cpK;
   final CopyConstructor<V> cpV;


   public BST(CopyConstructor<K>cpK, CopyConstructor<V> cpV) {
...
//--------------------------------------------------------------------------------
   ...
   public final boolean put(final K key, final V value) {
       ...
       K myKey = cpK.Copy(key);
       V myvalue = cpV.Copy(value);

       newNode = new Node<K, V>(myKey, myvalue);
           ...
           if (!(pinfo == null || pinfo.getClass() == Clean.class)) {
               ...
               if (l.key!= null && key.compareTo(l.key) == 0) {
                   ...
                   if (l.key == null || key.compareTo(l.key) < 0) // newinternal = max(ret.l.key, key);
                   {
                       ...
                       newInternal = new Node<K, V>(myKey, newSibling, newNode);
                   }
...
//--------------------------------------------------------------------------------
   ...
   public static void main(String[] args){
	    ...
	    BST<Buff,Buff> BuffTree = new BST<>(Buff.CC,Buff.CC);
	    Buff node = new Buff();
	    node.set(10);
