...
import com.yahoo.oak.Buff.Buff.MemSegmentReader;
...
public class LL_MemSeg<K,V> {
    ...
    public LL_MemSeg( NovaC<K> cmp,	NovaS<K> srz,
        ...
        try{
        	while (true) {
	            ...
	            if (curr.key != null && Kcm.compareKeys(curr.key, key) == 0) {
	    	    	...
	    	    	MemorySegment myK = MemorySegment.allocateNative(
	    	    			Ksr.calculateSize(key), ResourceScope.newSharedScope());
	    	    	MemorySegment myV = MemorySegment.allocateNative(
	    	    			Vsr.calculateSize(value), ResourceScope.newSharedScope());
    	...
    	try {
            while (true) {
                ...
                if(pred.next.compareAndSet(curr, succ, false, false)) {
                	curr.key.scope().close();
                	curr.value.scope().close();
                    return true;                	
                }
            }
    	}catch(IllegalStateException e) {continue CmpFail;}
