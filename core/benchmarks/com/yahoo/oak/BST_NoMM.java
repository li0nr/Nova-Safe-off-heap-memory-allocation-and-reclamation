       ...
       while (true) {
           ...
           if (l.key != null && KCt.compareKeys(l.key.address+l.key.offset, key)  == 0) {
               ...
               if (l.key == null ||  KCt.compareKeys(l.key.address+l.key.offset, key) > 0)	// newinternal = max(ret.l.key, key);
            	   ...
            	   NovaSlice Internal = new NovaSlice(0, 0, 0);
                   alloc.allocate(Internal, SrzK.calculateSize(key));
                   newInternal = new Node<NovaSlice, NovaSlice>(Internal, newSibling, newNode);
                   SrzK.serialize(key, newInternal.key.address +newInternal.key.offset );
               }

               final IInfo<NovaSlice, NovaSlice> newPInfo = new IInfo<NovaSlice, NovaSlice>(l, p, newInternal);
