...
import com.nova.Facade_Nova_MagicNumber;
 ...
 */
public class LL_Nova_primitive_noCAS_Magic<K,V> {
    ...
    public LL_Nova_primitive_noCAS_Magic(NovaManager novaManager,NovaC<K> cmp,	NovaS<K> srz,
    		NovaC<V> Vcmp,	NovaS<V> Vsrz) {	
		nm = novaManager; Kcm = cmp; Ksr = srz; Vcm = Vcmp; Vsr = Vsrz;
		new Facade_Nova_MagicNumber(nm);
        ...
        try{
        	while (true) {
                ...
                if (curr.key!= Illegal_nu && Facade_Nova_MagicNumber.Compare(key, Kcm, curr.key) == 0) {
                    ...
                    long OffVRef = Facade_Nova_MagicNumber.WriteFast(Vsr, value, Facade_Nova_MagicNumber.AllocateSlice(null, value_offset,
                    ...
                    if (pred.next.compareAndSet(curr, newNode, false, false)) {
                    	...
                    	Facade_Nova_MagicNumber.DeletePrivate(idx, newNode.key);
                    	Facade_Nova_MagicNumber.DeletePrivate(idx, newNode.value);
                    }
                }
            }  
    	...
    	try {
            while (true) {
                ...
                if(pred.next.compareAndSet(curr, succ, false, false)) {
                    Facade_Nova_MagicNumber.Delete(tidx, curr.value, curr, value_offset);
                    Facade_Nova_MagicNumber.Delete(tidx, curr.key, curr, key_offset);
                    return true;                	
                }
            }
    	}catch(NovaIllegalAccess e) {continue CmpFail;}
        		...
        		retry: 
        			while (true) {
	                    ...
	                    while (true) {
	                        ...
	                        if (curr == tail || Facade_Nova_MagicNumber.Compare(key, Kcm, curr.key) >= 0) { //we compare the offheap vs the key thus looking for >
    ...
    public boolean contains(K key, int tidx) {
        ...
        CmpFail: while(true)
        	try {
                ...
                while (curr != tail && Facade_Nova_MagicNumber.Compare(key, Kcm, curr.key) < 0) {
                    curr = curr.next.getReference();
                    curr.next.get(marked);
                }
                return curr.key == Illegal_nu ? false : Facade_Nova_MagicNumber.Compare(key, Kcm, curr.key) == 0 && !marked[0];
        	}catch (NovaIllegalAccess e) {continue CmpFail;}
    }
        ...
        CmpFail: while(true)
        	try {
                ...
                boolean flag =  curr.key == Illegal_nu ? 
                		false : Facade_Nova_MagicNumber.Compare(key, Kcm, curr.key) == 0 && !marked[0];
                R obj = null;
                if(flag) 
                	obj = (R) Facade_Nova_MagicNumber.Read(Reader, curr.value);
                return obj;
        	}catch (NovaIllegalAccess e) {continue CmpFail;}
    }
    ...
    public void Print() {
        Node curr = head.next.getReference();
        while (curr != tail ) {
 	       Facade_Nova_MagicNumber.Print(Kcm, curr.key);
 	       System.out.print("-->");
           curr = curr.next.getReference();
