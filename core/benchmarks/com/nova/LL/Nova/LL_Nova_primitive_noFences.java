 ...
 */
public class LL_Nova_primitive_noFences<K,V> {
    ...
    public LL_Nova_primitive_noFences(NovaManagerNoTap novaManager,NovaC<K> cmp,	NovaS<K> srz,
    		NovaC<V> Vcmp,	NovaS<V> Vsrz) {	
		nm = novaManager; Kcm = cmp; Ksr = srz; Vcm = Vcmp; Vsr = Vsrz;
		new Facade_Nova_FenceFree(nm);
        ...
        try{
        	while (true) {
                ...
                if (curr.key!= Illegal_nu && Facade_Nova_FenceFree.Compare(key, Kcm, curr.key) == 0) { 
                    ...
                    long OffKRef = Facade_Nova_FenceFree.WriteFast(Ksr, key, Facade_Nova_FenceFree.AllocateReusedSlice(newNode, key_offset,
                    		newNode.key, Ksr.calculateSize(key), idx),idx);
                    long OffVRef = Facade_Nova_FenceFree.WriteFast(Vsr, value, Facade_Nova_FenceFree.AllocateReusedSlice(newNode, value_offset,
                    ...
                    if (pred.next.compareAndSet(curr, newNode, false, false)) {
                    	...
                    	Facade_Nova_FenceFree.DeletePrivate(idx, newNode.key);
                    	Facade_Nova_FenceFree.DeletePrivate(idx, newNode.value);
                    }
                }
            }  
    	...
    	try {
            while (true) {
                ...
                if(pred.next.compareAndSet(curr, succ, false, false)) {
                    Facade_Nova_FenceFree.DeleteReusedSlice(tidx, curr.key, curr, key_offset);
                    Facade_Nova_FenceFree.DeleteReusedSlice(tidx, curr.value, curr, value_offset);
                    return true;                	
                }
            }
    	}catch(NovaIllegalAccess e) {continue CmpFail;}
        		...
        		retry: 
        			while (true) {
	                    ...
	                    while (true) {
	                        ...
	                        if (curr == tail || Facade_Nova_FenceFree.Compare(key, Kcm, curr.key) >= 0) { //we compare the offheap vs the key thus looking for >
    ...
    public <R> R get(K key,NovaR Reader, int tidx) {
        ...
        CmpFail: while(true)
        	try {
                ...
                while (curr != tail && Facade_Nova_FenceFree.Compare(key, Kcm, curr.key) < 0) {
                    curr = curr.next.getReference();
                    curr.next.get(marked);
                }
                boolean flag = curr.key == Illegal_nu ? false : Facade_Nova_FenceFree.Compare(key, Kcm, curr.key) == 0 && !marked[0];
                R obj = null;
                if(flag) 
                	obj = (R) Facade_Nova_FenceFree.Read(Reader, curr.value);
                return obj;
        	}catch (NovaIllegalAccess e) {continue CmpFail;}
    }
    ...
    public boolean contains(K key, int tidx) {
        ...
        CmpFail: while(true)
        	try {
                ...
                return curr.key == Illegal_nu ? false : Facade_Nova_FenceFree.Compare(key, Kcm, curr.key) == 0 && !marked[0];
        	}catch (NovaIllegalAccess e) {continue CmpFail;}
    }
    ...
    public void Print() {
        Node curr = head.next.getReference();
        while (curr != tail ) {
 	       Facade_Nova_FenceFree.Print(Kcm, curr.key);
 	       System.out.print("-->");
           curr = curr.next.getReference();
	...
	public static void main(String[] args) {
	    ...
	    final NovaManagerNoTap novaManager = new NovaManagerNoTap(allocator);
	    
	    Buff x =new Buff(4);
	    x.set(88);
	    LL_Nova_primitive_noFences<Buff, Buff>List = new LL_Nova_primitive_noFences<>(novaManager, Buff.DEFAULT_C, Buff.DEFAULT_SERIALIZER
