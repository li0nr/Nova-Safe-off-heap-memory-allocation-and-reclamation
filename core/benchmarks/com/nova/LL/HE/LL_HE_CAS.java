...
public class LL_HE_CAS<K,V> {
    ...
    public LL_HE_CAS(NativeMemoryAllocator allocator, NovaC<K> cmp,	NovaS<K> srz,
    ...
    public boolean add(K key, V value , int tidx) {
        ...
        while (true) {
            ...
            if (curr.key!= null && Facade_HE.Compare(key, Kcm, curr.key, tidx) == 0) { 
            	...
            	HEslice oKey  = Facade_HE.allocateCAS( Ksr.calculateSize(key));
        		Ksr.serialize(key, oKey.address+oKey.offset);
            	HEslice oValue  = Facade_HE.allocateCAS( Vsr.calculateSize(value));
    ...
    public boolean remove(K key, int tidx) {
    	...
    	while (true) {
            ...
            if(pred.next.compareAndSet(curr, succ, false, false)) {
            	Facade_HE.DeleteCAS(tidx, curr.key);
            	Facade_HE.DeleteCAS(tidx, curr.value);
                return true;	
                }
            }
    	}catch(NovaIllegalAccess e) {continue CmpFail;}
