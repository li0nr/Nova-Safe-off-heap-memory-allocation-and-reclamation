...
import com.nova.EBR;
import com.nova.Facade_EBR;
import com.nova.EBR.EBRslice;


public class LL_EBR_CAS <K,V>{
	    ...
	    public LL_EBR_CAS(NativeMemoryAllocator alloc ,NovaC<K> cmp,	NovaS<K> srz,
	    ...
	    public boolean add(K key, V value, int tidx) {
	    	...
	    	while (true) {
	            ...
	            if (curr.key != null && Kcm.compareKeys(curr.key.address + curr.key.offset, key) == 0) {
	            	...
	            	EBRslice oKey  = Facade_EBR.allocateCAS(Ksr.calculateSize(key));
	        		Ksr.serialize(key, oKey.address+oKey.offset);
	        		EBRslice oValue  = Facade_EBR.allocateCAS( Vsr.calculateSize(value));
