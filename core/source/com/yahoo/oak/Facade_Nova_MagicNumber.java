...
public class Facade_Nova_MagicNumber {
	...
	public Facade_Nova_MagicNumber(NovaManager mng) {
		novaManager = mng;
	}
	...
	static public <T> T Read(NovaR<T> lambda, long metadata) {
		...
		T obj = lambda.apply(address+offset+NovaManager.HEADER_SIZE+ NovaManager.MAGIC_SIZE);
	...
	static public <T>long WriteFast(NovaS<T> lambda, T obj, long facade_meta, int idx ) {//for now write doesnt take lambda for writing 
	 ...
	 static public <T> void Print(NovaC<T> srZ, long facademeta) {
			...
			srZ.Print(address+offset+NovaManager.HEADER_SIZE+ NovaManager.MAGIC_SIZE);
