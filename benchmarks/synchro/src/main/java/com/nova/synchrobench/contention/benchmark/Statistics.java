...
package com.nova.synchrobench.contention.benchmark;
...
public class Statistics {
    ...
    public enum AbortType {
        ALL, BETWEEN_SUCCESSIVE_READS, BETWEEN_READ_AND_WRITE, EXTEND_ON_READ, WRITE_AFTER_READ, LOCKED_ON_WRITE,
        LOCKED_BEFORE_READ, LOCKED_BEFORE_ELASTIC_READ, LOCKED_ON_READ, INVALID_COMMIT, INVALID_SNAPSHOT
    }
    ...
    private static final Map<Integer, Statistics> STATS_MAP = new HashMap<Integer, Statistics>();
    private static final int[] TX_ATTEMPTS_HIST_BINS;
    ...
    static {
        ...
        TX_ATTEMPTS_HIST_BINS = new int[histStrArr.length + 1];
        for (int i = 0; i < TX_ATTEMPTS_HIST_BINS.length - 1; i++) {
            String limitStr = histStrArr[i];
            int limit = Integer.parseInt(limitStr);
            TX_ATTEMPTS_HIST_BINS[i] = limit;
        }
        TX_ATTEMPTS_HIST_BINS[TX_ATTEMPTS_HIST_BINS.length - 1] = 100;
    }

    public static int getTotalStarts() {
        int totalStarts = 0;
        for (Map.Entry<Integer, Statistics> entry : STATS_MAP.entrySet()) {
    ...
    public static String getTotalAvgReadSetSize() {
        ...
        return formatDouble(average(sumOfAverages, STATS_MAP.size()));
    }
    ...
    public Statistics(int threadId) {
        STATS_MAP.put(threadId, this);
        txAttemptsHistCounters = new int[TX_ATTEMPTS_HIST_BINS.length];
    }
    ...
    public void reportCommit(int attempts) {
        ...
        if (startTime != -1) {
            ...
            for (int i = 0; i < TX_ATTEMPTS_HIST_BINS.length; i++) {
                int limit = TX_ATTEMPTS_HIST_BINS[i];
                if (attempts <= limit) {
