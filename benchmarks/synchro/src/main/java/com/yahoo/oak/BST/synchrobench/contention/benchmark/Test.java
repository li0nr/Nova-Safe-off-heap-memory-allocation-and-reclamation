...
import com.yahoo.oak.BST.synchrobench.contention.abstractions.MaintenanceAlg;
        ...
        switch (benchType) {
            case OAKMAP:
                ...
                for (short threadNum = 0; threadNum < Parameters.confNumThreads; threadNum++) {
                	threadLoopsBST[threadNum] = new ThreadLoop(threadNum, BST, methods);
...
//                + " %)");
        ...
        switch (benchType) {
            case OAKMAP:
                if (BST instanceof MaintenanceAlg) {
                    System.out.println("  #nodes (inc. deleted): \t"
                            + ((MaintenanceAlg) BST).numNodes());
                }
                break;
        }
